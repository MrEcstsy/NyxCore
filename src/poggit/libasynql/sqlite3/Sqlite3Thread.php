...
use main\src\poggit\libasynql\base\QueryRecvQueue;
use main\src\poggit\libasynql\base\QuerySendQueue;
use main\src\poggit\libasynql\base\SqlSlaveThread;
use main\src\poggit\libasynql\result\SqlChangeResult;
use main\src\poggit\libasynql\result\SqlColumnInfo;
use main\src\poggit\libasynql\result\SqlInsertResult;
use main\src\poggit\libasynql\result\SqlSelectResult;
use main\src\poggit\libasynql\SqlError;
use main\src\poggit\libasynql\SqlResult;
use main\src\poggit\libasynql\SqlThread;
...
class Sqlite3Thread extends SqlSlaveThread{
	...
	public static function createFactory(string $path) : Closure{
		return function(SleeperHandlerEntry $entry, QuerySendQueue $send, QueryRecvQueue $recv) use ($path){
			return new main\src\poggit\libasynql\sqlite3\Sqlite3Thread($path, $entry, $send, $recv);
		};
	}
	...
	protected function executeQuery($sqlite, int $mode, string $query, array $params) : SqlResult{
		...
		switch($mode){
			...
			case SqlThread::MODE_SELECT:
				/** @var main\src\poggit\libasynql\result\SqlColumnInfo[] $colInfo */
				$colInfo = [];
				$rows = [];
