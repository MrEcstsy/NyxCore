...
class VanillaEnchantmentsEvent implements Listener {
    ...
    public function onBreak(BlockBreakEvent $event): void
    {
        ...
        if (($level = $item->getEnchantmentLevel(EnchantmentIdMap::getInstance()->fromId($enchantment->getMcpeId()))) > 0) {
            ...
            foreach (Nyx::getInstance()->getConfig()->get("fortune.blocks", []) as $str) {
        ...
        if ($killer instanceof Player) {
            ...
            if (($level = $killer->getInventory()->getItemInHand()->getEnchantmentLevel(EnchantmentIdMap::getInstance()->fromId($lootingEnchantment->getMcpeId()))) > 0) {
                if (
                    ...
                    $add = mt_rand(0, $level + 1);
                    if (is_bool(Nyx::getInstance()->getConfig()->get("looting.entities"))) {
                        Server::getInstance()->getLogger()->debug("There is an error (looting) in the config of vanillaEC");
                        return;
                    }
                    $lootingMultiplier = Nyx::getInstance()->getConfig()->get("looting.drop_multiplier", 1); // Drop multiplier from config

                    foreach (Nyx::getInstance()->getConfig()->get("looting.entities", []) as $items) {
